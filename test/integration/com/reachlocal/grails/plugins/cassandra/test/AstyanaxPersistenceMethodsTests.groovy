...
class AstyanaxPersistenceMethodsTests
{
	...
	void testPutAndGetColumn()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def col = mapping.getColumn(keyspace, columnFamily, key, "name", null)
	...
	void testPutAndGetRow()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def row = mapping.getRow(keyspace, columnFamily, key, "CL_ONE")
	...
	void testPutAndGetRows()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			...
			def rows = mapping.getRows(keyspace, columnFamily, keys, "CL_ONE")
	...
	void testPutAndGetRowsWithEqualityIndex()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def rows = mapping.getRowsWithEqualityIndex(keyspace, columnFamily, [city: "City 1", state:  state], 100, null)
			assertEquals 2, rows.size()

			rows = mapping.getRowsWithEqualityIndex(keyspace, columnFamily, [city: "City 3", state:  state], 100, null)
	...
	void testPutAndCountRowsWithEqualityIndex()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def count = mapping.countRowsWithEqualityIndex(keyspace, columnFamily, [city: "City 1", state:  state], null)
			assertEquals 2, count

			count = mapping.countRowsWithEqualityIndex(keyspace, columnFamily, [city: "City 2", state:  state], null)
	...
	void testPutAndGetRowsColumnSlice()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			...
			def rows = mapping.getRowsColumnSlice(keyspace, columnFamily, keys, ["name","x5"], null)
			assertEquals 2, rows.size()
	...
	void testPutAndGetColumnRange()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def range = mapping.getColumnRange(keyspace, columnFamily, key, null, null, false, 10, null)
			assertEquals 6, range.size()

			range = mapping.getColumnRange(keyspace, columnFamily, key, null, null, false, 3, null)
			assertEquals 3, range.size()

			range = mapping.getColumnRange(keyspace, columnFamily, key, "x1", "x4", false, 10, null)
			assertEquals 4, range.size()
			assertEquals "one", mapping.stringValue(range.getColumnByIndex(0))

			range = mapping.getColumnRange(keyspace, columnFamily, key, "x4", "x3", true, 10, null)
	...
	void testPutAndCountColumnRange()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def count = mapping.countColumnRange(keyspace, columnFamily, key, null, null, null)
			assertEquals 6, count

			count = mapping.countColumnRange(keyspace, columnFamily, key, "x1", "x4", null)
			assertEquals 4, count

			count = mapping.countColumnRange(keyspace, columnFamily, key, "x3", "x4", null)
			assertEquals 2, count
		}
	}
	...
	void testPutAndGetColumnSlice()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def slice = mapping.getColumnSlice(keyspace, columnFamily, key, ["name","x5"], null)
			assertEquals 2, slice.size()
			assertEquals "five", mapping.stringValue(slice.getColumnByIndex(1))

			slice = mapping.getColumnSlice(keyspace, columnFamily, key, ["x2"], null)
	...
	void testPutAndDeleteColumn()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def row = mapping.getRow(keyspace, columnFamily, key, null)
			assertEquals "Test User 1", mapping.stringValue(mapping.getColumn(row, "name"))
	...
	void testIncrementAndGetColumn()
	{
		...
		astyanaxService.withKeyspace() {keyspace ->
			def col1 = mapping.getColumn(keyspace, counterColumnFamily, key, "places", null)
			assertEquals 1, mapping.longValue(col1)
			def col2 = mapping.getColumn(keyspace, counterColumnFamily, key, "books", null)
